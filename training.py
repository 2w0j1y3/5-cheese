...
class ZobristQLearningAI:
    def __init__(self, player, learning_rate=0.1, discount_factor=0.95, 
        ...
        # 用于动态学习率调整的近期结果记录
        self.recent_outcomes = []   # 存储每局自己的结果: 'win', 'loss', 'draw'
        self.max_recent = 100
    ...
    def adjust_learning_rate(self, target_win_rate=0.5, adjust_factor=0.05):
        """根据近期胜率动态调整学习率"""
        ...
        if win_rate > target_win_rate + 0.05:   # 胜率高于目标5个百分点，降低学习率
            self.lr = max(0.001, self.lr * (1 - adjust_factor))
        elif win_rate < target_win_rate - 0.05: # 胜率低于目标5个百分点，提高学习率
            self.lr = min(0.5, self.lr * (1 + adjust_factor))
        # 否则保持不变
    ...
    def load(self, filename):
        ...
        self.recent_outcomes = save_data.get('recent_outcomes', [])
...
class AITrainer:
    ...
    def play_game(self, first_player=None, training=True):
        ...
        while not done and move_count < MAX_MOVES:
            ...
            # ---- 新增聚拢奖励 ----
            if training and reward == 0:   # 只在非终结步添加，避免掩盖胜负信号
                # 计算新落子周围2格内的己方棋子数（不包括自身）
                row, col = action
                friendly_count = 0
                for i in range(max(0, row-2), min(self.board_size, row+3)):
                    for j in range(max(0, col-2), min(self.board_size, col+3)):
                        if (i, j) != (row, col) and env.board[i, j] == current_ai.player:
                            friendly_count += 1
                cluster_bonus = 0.1 * friendly_count
                reward += cluster_bonus
            # --------------------
        ...
        if done:
            if reward == 1:
                ...
                # 记录个人结果
                winner.record_game_result('win')
                loser.record_game_result('loss')
            elif reward == 0:
                self.ai1.draws += 1
                self.ai2.draws += 1
                result = 'draw'
                self.ai1.record_game_result('draw')
                self.ai2.record_game_result('draw')
            else:
    ...
    def train(self, episodes=100000, save_every=10000, log_every=500):
        ...
        lr_adjust_interval = 100   # 动态学习率调整间隔
        ...
        for episode in range(1, episodes + 1):
            ...
            # ---- 新增动态学习率调整 ----
            if episode % lr_adjust_interval == 0:
                self.ai1.adjust_learning_rate()
                self.ai2.adjust_learning_rate()
            # --------------------------
