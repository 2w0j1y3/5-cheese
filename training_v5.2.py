...
class Gomoku:
    ...
    def is_opponent_threat(self, row, col, opponent):
        ...
        if threats['live_four'] > 0:
            return True
        if threats['live_three'] >= 2:          # 双活三
            return True
        if threats['live_three'] >= 1 and threats['rush_four'] >= 1:  # 冲四活三
            return True
        if threats['rush_four'] >= 2:            # 双冲四（可选，为保险保留）
            return True
        return False
    ...
    def _evaluate_direction(self, row, col, player, dr, dc, is_opponent=False):
        ...
        if count >= 5:
            ...
            if not (left_block and right_block):
                base_score += 500000 if is_opponent else 200000   # 对手活四
            else:
                base_score += 100000 if is_opponent else 20000    # 对手冲四
        elif count == 3:
            if not (left_block and right_block):
                base_score += 200000 if is_opponent else 80000    # 对手活三
            else:
                base_score += 30000 if is_opponent else 8000      # 对手冲三
        elif count == 2:
            if not (left_block and right_block):
...
class ZobristQLearningAI:
    def __init__(self, player, learning_rate=0.1, discount_factor=0.95, 
                ...
                max_q_size=5000000, prune_keep_size=2000000):
        self.player = player
        ...
        # 进攻/防守/多余权重（综合Q值计算用）
        self.attack_weight = 0.4      # 提高初始进攻权重
        self.defend_weight = 0.5      # 适当降低防守权重
        self.attack_heuristic = 0.001   # 动作选择时进攻启发式系数
        self.defend_heuristic = 0.0005  # 动作选择时防守启发式系数
        self.attack_reward_coef = 0.001 # 额外奖励中进攻系数
        self.defend_reward_coef = 0.0005# 额外奖励中防守系数
        self.zobrist = ZobristHash()
        
        # 新Q表结构：每个状态-动作存储2个值
        self.q_table = {}  # key: state_hash, value: dict {action: (attack_q, defend_q)}
        
        self.max_q_size = max_q_size
        self.prune_keep_size = prune_keep_size
    ...
    def get_action(self, state, valid_moves, training=True):
        """选择动作 - 优先防守对手的活三/冲四等威胁，并引入强制进攻探索"""
        ...
        # 强制进攻探索：20%的概率直接选择进攻分最高的动作（仅在训练时）
        ...
        for move in valid_moves:
            # 获取二维Q值，默认为 [0.0, 0.0]
            q_att, q_def = self.q_table[state_hash].get(move, [0.0, 0.0])
    ...
    def _detect_all_threats(self, env, opponent):
        """
        检测对手下一步可能下的所有威胁位置
        返回位置集合，这些位置如果被对手占据，将形成活三、冲四、双三等必防局面
        """
        threats = set()
        board = env.board
        ...
        for i in range(env.board_size):
            for j in range(env.board_size):
                if board[i, j] == 0:
                    ...
                    # 创建临时环境用于检查
                    temp_env = Gomoku(env.board_size)
                    temp_env.board = temp_board
                    # 检查是否形成威胁
                    if self._is_threatening(temp_env, i, j, opponent):
                        threats.add((i, j))
        return threats

    def _is_threatening(self, env, row, col, player):
        """利用环境对象的精确威胁检测方法"""
        return env.is_opponent_threat(row, col, player)

    def update(self, state, action, reward, next_state, done):
        state_hash = self.get_state_hash(state)
        if state_hash not in self.q_table:
            self.q_table[state_hash] = {}
        current = self.q_table[state_hash].get(action, [0.0, 0.0])
        current_q_att, current_q_def = current
        ...
        self.q_table[state_hash][action] = [new_q_att, new_q_def]
    ...
    def _prune_q_table(self):
        """修剪Q表，保留约2/3的状态（随机抽样）"""
        if len(self.q_table) <= self.prune_keep_size:
            return
        items = list(self.q_table.items())
        random.shuffle(items)
        self.q_table = dict(items[:self.prune_keep_size])
        logger.info(f"Q表已修剪: {len(items)} -> {self.prune_keep_size}")
    ...
    def save(self, filename, total_training_games=None):
        ...
        save_data = {
            ...
            'prune_keep_size': self.prune_keep_size,
    ...
    def load(self, filename):
        ...
        self.prune_keep_size = save_data.get('prune_keep_size', self.prune_keep_size)
...
class AITrainer:
    def __init__(self, ai1, ai2, board_size=BOARD_SIZE):
        ...
        self.global_aggressiveness = 1.0   # 全局进攻倾向系数
        self.high_draw_counter = 0          # 连续高平局计数
    ...
    def _balance_ai(self):
        ...
        if draw_rate > 80:
            ...
            # 平局过少：温和加强防守（不再强力重置）
            factor = 0.95  # 防守加强因子（攻击乘0.95，防守除0.95 ≈ 防守乘1.05）
    ...
    def plot_stats(self, episode):
        """绘制统计图 - 6张图：胜负统计、胜率趋势（含差距）、探索率、学习率、进攻权重、防守权重"""
        try:
            ...
            total_safe = np.where(total == 0, 1, total)  # 避免除零
