...
class DQNAI:
    def __init__(self, player, learning_rate=0.001, discount_factor=0.95,
                 ...
                 attack_weight=0.4, defend_weight=0.5,  # 保留兼容，但不用于Q值计算
    ...
    def _state_to_tensor(self, state):
        """
        将原始状态 (board, player) 转换为网络输入张量 (3, 9, 9)
        通道0: 当前玩家棋子
        通道1: 对手棋子
        通道2: 当前玩家标志（全1表示黑方回合，全0表示白方回合）
        """
        ...
        return torch.from_numpy(tensor).unsqueeze(0).to(DEVICE)  # 添加batch维度

    def get_action(self, state, valid_moves, training=True):
        """
        选择动作：先检测对手威胁强制防守，否则用网络Q值 + ε-greedy
        """
    ...
    def save(self, filename, total_training_games=None):
        ...
        logger.info(f"模型已保存: {Path(filename).name} ({size_mb:.2f} MB)")
...
class AITrainer:
    ...
    def _detect_urgent_defense(self, env, opponent, ai):
        ...
        return list(threats)
    # 只需注意在_balance_ai中调整学习率时，需要同步更新优化器的学习率
...
def main():
    ...
    if models:
        ...
        # 创建DQN AI
        ai_black = DQNAI(player=1, learning_rate=0.001, exploration_rate=0.4)
        ai_white = DQNAI(player=2, learning_rate=0.001, exploration_rate=0.4)
